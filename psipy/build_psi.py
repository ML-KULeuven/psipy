...
def rename_PSIutil():
    ...
    for f_in in os.listdir(psipath):
        if f_in[-2:]=='.d' or f_in[-3:]=='.sh':
            ...
            if f_in=='util.d':
                os.rename(os.path.join(psipath,'util.d'), os.path.join(psipath,'dutil.d'))
...
def build_psi():
    ...
    subprocess.call("dmd -O -release -inline -boundscheck=off -J./library *.d -fPIC -lib -of={lib_name}".format(lib_name=lib_name)\
                        , shell=True, cwd=os.path.join(path, "psi"))
